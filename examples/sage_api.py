...
parser.add_argument("--azure_endpoint", default="https://openai-api-aiapp-usest.openai.azure.com", help="Azure OpenAI endpoint")
parser.add_argument("--api_version", default="2025-01-01-preview", help="Azure OpenAI API version")
parser.add_argument("--use_azure", action="store_true", default=True, help="使用 Azure OpenAI")
...
default_model_client: Optional[Union[OpenAI, AzureOpenAI]] = None
...
async def initialize_system(server_args):
    ...
    try:
        ...
        if server_args.default_llm_api_key:
            if server_args.use_azure or server_args.azure_endpoint:
                default_model_client = AzureOpenAI(
                    api_key=server_args.default_llm_api_key,
                    azure_endpoint=server_args.azure_endpoint or server_args.default_llm_api_base_url,
                    api_version=server_args.api_version
                )
                logger.info(f"Azure OpenAI 客户端初始化成功: {server_args.default_llm_model_name}")
            else:
                default_model_client = OpenAI(
                    api_key=server_args.default_llm_api_key,
                    base_url=server_args.default_llm_api_base_url
                )
                logger.info(f"OpenAI 客户端初始化成功: {server_args.default_llm_model_name}")
...
async def stream_chat(request: StreamRequest):
    ...
    if request.llm_model_config or request.system_prefix or request.available_tools:
        ...
        # 检查是否使用 Azure OpenAI
        if server_args.use_azure or server_args.azure_endpoint or 'azure.com' in base_url:
            model_client = AzureOpenAI(
                api_key=api_key,
                azure_endpoint=server_args.azure_endpoint or base_url,
                api_version=server_args.api_version
            )
        else:
            model_client = OpenAI(
