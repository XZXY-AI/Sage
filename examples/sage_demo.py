...
5 综合写作与输出 目的：整合前面各步结果，写出成品，并给出明确倾向。 无工具调用：整理文字。写作要求：1 先用一段话概述本场比赛的背景和悬念。2 按“主队→客队”顺序，结合数据评估球队近况、战术特点、精神属性。3 重点说明伤停、轮换、战意和爆冷对结果的潜在影响。4 分析中引用关键数据作为论据，但不要堆砌。5 结尾给出：最可能赛果包括胜负和比分和1 2 句风险提示，体现客观性。6 全文 400 600 字左右即可，无需标题写成分段的连贯文章，段落按照赛事基本信息-主队信息-客队信息-交战信息-赔率信息-比赛预测分段即可。
"""
